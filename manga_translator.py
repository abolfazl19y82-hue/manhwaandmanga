...
# --- تنگ‌کردنِ ماسکِ متن قبل از پاکسازی (اختیاری) ---
# اگه فایل mask_refine_v2.py کنار این اسکریپت باشه فعال می‌شه؛ در غیر این
# صورت بی‌سروصدا نادیده گرفته می‌شه و پاکسازی مثل قبل کار می‌کنه.
...
class MangaTranslator:
    ...
    def __init__(
        ...
        # فایل مرجع ثابت و قابل‌ویرایش (پیش‌فرض: translation_reference.txt).
        # فقط یه بار از روی دیسک خونده می‌شه (بدون هیچ Request به Gemini) و
        # محتواش عیناً به‌عنوان بخشی از همون Request ترجمه‌ی موجود به مدل
        # داده می‌شه؛ هیچ Request جداگانه‌ای برای این کار ایجاد نمی‌شه.
        self.reference_path: Optional[str] = reference_path
        self.reference_text: str = ""
        # mask_refine_mode: "auto" (پیش‌فرض: ماسکِ متن قبل از پاکسازی، اگه
        # ماژول mask_refine_v2 موجود بود، تنگ‌تر می‌شه)، "legacy" (بدون
        # تنگ‌کردن)، یا "crf" (اجباری، نیازمند pydensecrf).
    ...
    def _refine_text_mask_tight(self, image: np.ndarray, raw_mask: np.ndarray) -> np.ndarray:
        """قبل از flat-fill/inpaint، ماسکِ متن رو (اگه ماژول mask_refine_v2
        در دسترس بود) تنگ‌تر می‌کنه. هیچ‌وقت raise نمی‌کنه (به‌جز حالت
        اجباریِ 'crf' وقتی pydensecrf نصب نیست) و هر خطایی باعث می‌شه دقیقاً
        همون raw_mask قبلی برگرده."""
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            # اگه چندضلعی‌های دقیقِ خودِ متن (از OCR روی برشِ حباب) در دسترس
            # باشه، ماسک از روی همون‌ها ساخته می‌شه (تنگ، فقط دور حروف)؛
            # وگرنه (مسیر fallback تمام‌صفحه که خودِ boxes همون خطوط OCR
            # تشخیص‌داده‌شده‌ست) از boxes استفاده می‌شه. این از پرشدن/inpaint
            # کل مستطیلِ حبابِ تشخیص‌داده‌شده توسط RT-DETR جلوگیری می‌کنه که
            # باعث لکه‌های مستطیلی روی طرح دور بالن‌های گرد/نامنظم می‌شد.
            tight_polys = list(getattr(region, "ocr_polys", None) or [])
            polys_for_mask = tight_polys if tight_polys else region.boxes
            for poly in polys_for_mask:
        ...
        pad_iterations = max(1, int(self.mask_padding))
        dilated = cv2.dilate(full_target_mask, kernel, iterations=pad_iterations)
        return dilated
    ...
    def _flat_fill_uniform_components(
        ...
        mask: np.ndarray,
        ring_px: int = 7,
        std_thresh: float = 13.0,
        min_ring_pixels: int = 40,
        feather_px: int = 2,
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        قبل از رفتن سراغ inpaint، هر بلوکِ متنِ ماسک‌شده رو جدا بررسی می‌کنه:
        ...
        نتیجه‌ی خیلی تمیزتر از inpaint. اگه زمینه بافت‌دار/رنگارنگ بود، اون
        بخش رو دست‌نخورده تو ماسکِ باقی‌مونده نگه می‌داره تا با inpaint پاک
        بشه.
        ...
        if self.use_lama:
            ...
            if lama is not None:
                try:
                    ...
                    result_pil = lama(flat_filled, remaining_mask)
                    result_bgr = cv2.cvtColor(np.array(result_pil), cv2.COLOR_RGB2BGR)
                    print(f"  - پاکسازی نواحی باقی‌مونده با {getattr(self, '_inpainter_name', 'ONNX')} انجام شد.")
                    return result_bgr
                except Exception as e:
                    print(f"  [!] {getattr(self, '_inpainter_name', 'ONNX')} خطا داد ({e})؛ به OpenCV برمی‌گردیم.")
    ...
    # ------------------------------------------------------------------
    # فایل مرجع ثابت (translation_reference.txt) — اسم‌ها/اصطلاح‌ها
    # ------------------------------------------------------------------
    def set_reference_path(self, path: Optional[str]) -> None:
        self.reference_path = path

    def load_reference_file(self, path: Optional[str] = None) -> None:
        """فایل مرجع ثابت (پیش‌فرض translation_reference.txt) رو یک‌بار از
        روی دیسک می‌خونه و توی self.reference_text نگه می‌داره. این متد فقط
        فایل رو می‌خونه؛ هیچ Request به Gemini نمی‌فرسته. اگه فایل وجود
        نداشته باشه یا خالی باشه، بی‌سروصدا رد می‌شه."""
        ...
        try:
            ...
            print(f"    [!] نتونستم فایل مرجع رو از {path} بخونم: {e}")
            return
        if not text:
            return
        kept_lines = [ln for ln in text.splitlines()
                      if ln.strip() and not ln.strip().startswith("#")]
        self.reference_text = "\n".join(kept_lines)
        if not self.reference_text:
            return
        print(f"[*] فایل مرجع بارگذاری شد: {len(kept_lines)} خط از {path} "
              f"(بدون Request جدا؛ همراه ترجمه ارسال می‌شه)")

    def _reference_prompt_block(self) -> str:
        """بلوک متنی فایل مرجع رو برای تزریق به سیستم‌اینستراکشن ترجمه
        می‌سازه. اگه فایل مرجع خالی/موجود نباشه، رشته‌ی خالی برمی‌گرده."""
        if not self.reference_text:
            return ""
        return (
            "\n\n━━━━━━━━━━━━━━━━━━━━\n"
            "📚 فایل مرجع ثابت پروژه (translation_reference.txt) — "
            "اولویت مطلق برای اسم‌ها/اصطلاح‌ها\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "این فایل یک فایل مرجع برای نام‌ها و اصطلاحات این اثر است. هر "
            "اسم، اصطلاح، لقب، مهارت، مکان یا عبارت موجود در این فایل باید "
            "دقیقاً مطابق معادل تعیین‌شده در فایل ترجمه شود؛ برای موارد "
            "موجود در آن، ترجمه‌ی دیگری انتخاب نکن. اگر موردی در این فایل "
            "نبود، طبق روال معمول و متناسب با بافت ترجمه کن.\n"
            "محتوای فایل مرجع (هر خط: «متن اصلی = معادل فارسی»):\n"
    ...
    def _ocr_crop(self, image_bgr: np.ndarray, rect) -> Tuple[str, List[np.ndarray]]:
        ...
        h, w = image_bgr.shape[:2]
        # حاشیه‌ی نسبی (به‌جای فقط ۴ پیکسل ثابت): وقتی RT-DETR باکسِ بالن رو
        # تنگ‌تر از بالنِ واقعی تشخیص بده، متنِ نزدیکِ لبه هم داخل برش می‌افته
        # (مطابق روش پروژه‌ی اول که به مستطیل حباب اصلاً وابسته نیست).
        bw0, bh0 = max(1, x2 - x1), max(1, y2 - y1)
        pad_x = max(4, int(bw0 * 0.18))
        pad_y = max(4, int(bh0 * 0.18))
        ...
        # بزرگ‌نماییِ برش‌های خیلی کوچیک قبل از OCR (دقت روی فونت ریز رو بالا
        # می‌بره)؛ چون برش‌ها کوچیکن، این کار عملاً هزینه‌ی محسوسی نداره.
        ch, cw = crop.shape[:2]
        scale = 1.0
        if max(ch, cw) < 220:
            scale = 260.0 / max(1, max(ch, cw))
        if scale > 1.05:
            crop_proc = cv2.resize(crop, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        else:
            crop_proc = crop
            scale = 1.0

        # فاز چندپاسه (مثل two_pass_ocr پروژه‌ی اول) ولی فقط رو همین برشِ
        # کوچیک، نه کل صفحه -> هزینه‌ی محسوسی به سرعت کل صفحه اضافه نمی‌کنه.
        detections: List[dict] = []
        try:
            detections.extend(self.detect_text(crop_proc))
        except Exception:
            pass
        if self.two_pass_ocr:
            try:
                detections.extend(self.detect_text(self._clahe_enhance(crop_proc)))
            except Exception:
                pass
            try:
                detections.extend(self.detect_text(cv2.bitwise_not(crop_proc)))
            except Exception:
                pass
            try:
                gray = cv2.cvtColor(crop_proc, cv2.COLOR_BGR2GRAY)
                _, bwimg = cv2.threshold(gray, 160, 255, cv2.THRESH_BINARY)
                if float(np.mean(bwimg)) < 127:
                    bwimg = cv2.bitwise_not(bwimg)
                bwimg = cv2.dilate(bwimg, np.ones((2, 2), np.uint8), iterations=1)
                detections.extend(self.detect_text(cv2.cvtColor(bwimg, cv2.COLOR_GRAY2BGR)))
            except Exception:
                pass
        ...
        # ترتیبِ طبیعیِ خواندنِ داخلِ یک بالن: بالا به پایین، بعد چپ به راست
        detections.sort(key=lambda d: (cv2.boundingRect(d["poly"])[1], cv2.boundingRect(d["poly"])[0]))
        ...
        for d in detections:
            ...
            if not text:
                continue
            lines.append(text)
            poly = np.asarray(d["poly"], dtype=np.int32).reshape(-1, 2).astype(np.float32).copy()
            if scale != 1.0:
                poly /= scale
    ...
    def _extract_regions_from_bubbles(self, image: np.ndarray) -> List[TextRegion]:
        ...
        for i, b in enumerate(boxes):
            ...
            # rect (که برای اندازه‌ی فونتِ رندر و مبنای پاکسازی استفاده می‌شه)
            # از رو محلِ واقعیِ متنِ تشخیص‌داده‌شده ساخته می‌شه، نه مستطیلِ خامِ
            # RT-DETR — دقیقاً مثل پروژه‌ی اول که اصلاً به حدسِ بالن وابسته
            # نیست. این از سرریزِ متنِ فارسی بیرون از بالن (وقتی RT-DETR
            # مستطیل رو تنگ‌تر از بالنِ واقعی می‌گیره) جلوگیری می‌کنه.
            if line_polys:
                all_pts = np.concatenate([p.reshape(-1, 2) for p in line_polys], axis=0)
                tx0, ty0 = int(all_pts[:, 0].min()), int(all_pts[:, 1].min())
                tx1, ty1 = int(all_pts[:, 0].max()), int(all_pts[:, 1].max())
                rpad_x = max(4, int((tx1 - tx0) * 0.10))
                rpad_y = max(4, int((ty1 - ty0) * 0.18))
                rx0, ry0 = max(0, tx0 - rpad_x), max(0, ty0 - rpad_y)
                rx1, ry1 = min(w, tx1 + rpad_x), min(h, ty1 + rpad_y)
    ...
    @staticmethod
    def _save_as_zip(image_paths_in_order: List[str], out_path: str) -> None:
        os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    ...
    def _write_image(self, image: np.ndarray, path: str) -> None:
        ...
        if ext == ".webp":
            # نکته‌ی مهم: WebP در حالتِ lossy همیشه chroma subsampling رو
            # روی 4:2:0 قفل می‌کنه (برخلاف JPEG که در همین فایل، وقتی
            # q>=90 باشه subsampling=0 می‌گیره). همین باعث می‌شد لبه‌ی
            # متن‌ها و خط‌های ریز مانگا توی خروجی ZIP (که مستقیماً همین
            # فایل‌ها رو zip می‌کنه) محوتر از PDF به نظر برسه؛ چون PDF
            # این فایل‌ها رو دوباره به JPEG با subsampling=0 تبدیل می‌کنه.
            # راه‌حل: با همون منطقِ q>=90 که برای JPEG هست، این‌جا هم به
            # جای lossy quality=90 از حالتِ lossless استفاده می‌کنیم تا
            # هیچ افتِ کیفیتی رخ نده (حجم فایل بیشتر می‌شه ولی افت کیفیت
            # صفر می‌شه).
            if q >= 90:
                pil.save(
                    path, format="WEBP", lossless=True, method=6,
                    exact=False,
                )
            else:
                pil.save(
...
"""
    ...
    def _stitch_pages_for_efficiency(self, image_files: List[str], work_dir: str) -> List[str]:
        ...
        # نکته‌ی مهم: قبلاً اینجا فقط از ۸ صفحه‌ی اول نمونه‌برداری می‌شد و
        # عرضِ «میانه» به‌عنوان معیار انتخاب می‌شد. این باعث می‌شد صفحاتی
        # که رزولوشن‌شون بالاتر از میانه بود، موقعِ چسباندن به عرضِ کمتر
        # downscale بشن و کیفیت‌شون افت کنه. برای اینکه هیچ صفحه‌ای هیچ‌وقت
        # به کیفیتِ پایین‌تر downscale نشه، الان عرضِ *همه‌ی* صفحاتی که قراره
        # چسبونده بشن رو می‌خونیم (فقط هدر فایل، بدون دیکود کامل تصویر، پس
        # سریعه) و بزرگ‌ترین عرض رو به‌عنوان معیار مشترک انتخاب می‌کنیم.
        # صفحاتِ کوچیک‌تر upscale می‌شن (که چیزی از جزئیاتِ اصلی حذف نمی‌کنه)
        # و صفحاتِ باکیفیت‌تر دست‌نخورده می‌مونن.
        sample_widths = []
        for f in image_files[start_idx:]:
            try:
                with Image.open(f) as imf:
                    sample_widths.append(imf.size[0])
            except Exception:
                continue
        if not sample_widths:
            return image_files
        target_w = max(sample_widths)
    ...
    def run(self, input_path: str, output_path: str, resume: bool = True,
        ...
        if not self.reference_path:
            self.reference_path = "translation_reference.txt"
        ...
        # نکته‌ی مهم: این بلوکِ «نرمال‌سازی هوشمندِ عرض» قبلاً *همیشه* اجرا
        # می‌شد و همه‌ی صفحه‌ها رو (حتی وقتی --max-width 0 یعنی خاموش بود)
        # به یه عرضِ خوشه‌بندی‌شده resize می‌کرد — که باعثِ افتِ کیفیتِ محسوس
        # روی صفحاتی می‌شد که اصلاً نیازی به تغییر اندازه نداشتن. پروژه‌ی اول
        # اصلاً چنین مرحله‌ای نداره؛ الان این بلوک فقط وقتی اجرا می‌شه که
        # کاربر واقعاً --max-width بزرگ‌تر از صفر مشخص کرده باشه.
        if self.max_output_width and self.max_output_width > 0 and len(image_files) >= 1:
            widths: List[int] = []
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--reference-file", default="translation_reference.txt",
                   help="مسیر فایل مرجع ثابت اسم‌ها/اصطلاح‌ها/لقب‌ها (translation_reference.txt).")
...
def main():
    ...
    translator = MangaTranslator(
        ...
        reference_path=getattr(args, "reference_file", None),
    )
